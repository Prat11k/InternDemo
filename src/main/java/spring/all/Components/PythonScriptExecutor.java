...
public class PythonScriptExecutor implements ScriptExecutor {
	...
	public String execute(String scriptPath) throws ScriptExecutorException {
		scriptExecutorFactory.createScriptExecutor("python", scriptPath);
		try {
			logger.info("Executing Python script: {}", scriptPath);
			ProcessBuilder processBuilder = new ProcessBuilder("py", scriptPath);
			...
			throw new ScriptExecutorException("Python script execution failed");
		}
